...
export default function WorkoutSummaryModal({
  ...
  // Pre-compute random-derived values when modal opens
  const [randomOffset, setRandomOffset] = useState({ hr: 0, score: 0, msgIdx: 0 });
  useEffect(() => {
    if (open) {
      setRandomOffset({
        hr: Math.floor(Math.random() * 12),
        score: Math.floor(Math.random() * 8),
        msgIdx: Math.floor(Math.random() * 100),
      });
    }
  }, [open]);
  ...
  const evalMsg = evalMsgs[randomOffset.msgIdx % evalMsgs.length].replace(/\{action\}/g, workout.currentAction);

  const peakHR = Math.min(185, biometrics.heartRate + randomOffset.hr);
  const avgScore = Math.max(55, workout.score - randomOffset.score);
