...
import { CheckCircle2, ChevronDown, ChevronRight, Play, Video, X } from 'lucide-react';
...
// Preset videos — shipped with the project in public/uploads/coach-videos/
const PRESET_VIDEOS: PresetCoachVideo[] = [
  {
    recordingId: 'pamela-12min-slim-legs',
    title: '帕梅拉 12分钟瘦腿训练',
    localPath: '/uploads/coach-videos/pamela-12min-slim-legs.mp4',
    hasSkeleton: false,
  },
  {
    recordingId: 'pamela-10min-cardio-bottle',
    title: '帕梅拉 10分钟活力有氧+水瓶',
    localPath: '/uploads/coach-videos/pamela-10min-cardio-bottle.mp4',
    hasSkeleton: false,
  },
  {
    recordingId: 'pamela-10min-abs-legs',
    title: '帕梅拉 10分钟站立瘦腹+纤腿',
    localPath: '/uploads/coach-videos/pamela-10min-abs-legs.mp4',
    hasSkeleton: false,
  },
  {
    recordingId: 'pamela-15min-jumping-cardio',
    title: '帕梅拉 15分钟跳跃有氧',
    localPath: '/uploads/coach-videos/pamela-15min-jumping-cardio.mp4',
    hasSkeleton: false,
  },
  {
    recordingId: 'zhouye-10min-standing-abs',
    title: '周六野 10分钟站立马甲线瘦腰',
    localPath: '/uploads/coach-videos/zhouye-10min-standing-abs.mp4',
    hasSkeleton: false,
  },
];
...
export default function CoachVideoUploader({
  ...
  const handleSelect = (video: PresetCoachVideo) => {
    setSelected(video);
    setExpanded(true);
    onUploadComplete(video.recordingId, video.localPath);
  };
  ...
  const handleStart = () => {
    if (selected) onStartFollowAlong(selected.recordingId, selected.localPath);
  };
