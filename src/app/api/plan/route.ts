...
export async function POST(req: NextRequest) {
  try {
    ...
    const { age, fitnessLevel, goal, heartRate, currentExercise, personality } = body as {
      age: number;
      fitnessLevel: string;
      goal: string;
      heartRate: number;
...
根据用户的健康档案和实时心率，生成一份简洁的今日训练计划。

要求：
1. 用2-3句话评估当前身体状态
2. 给出3-5条具体训练建议（包括组数、时长、心率目标区间）
3. 如果心率过高，建议休息或降强度
4. 如果心率未连接，提醒连接Apple Health获取实时心率
5. 语言风格要跟教练人设一致
...
7. 总长度不超过150字`;
    ...
    const userPrompt = [
      ...
      heartRate > 0 ? `当前心率：${heartRate} BPM（最大心率${maxHR}的${hrPercent}%）` : '心率：未连接（请连接Apple Health）',
